...
const Navbar = () => {
  ...
  return (
    <motion.nav
      ...
      initial="hidden"
      animate="show"
      className={`w-full max-w-4xl mx-auto px-4 md:px-8 py-3 flex items-center justify-between sticky top-0 z-20 ${
        changeNavbar ? "border-b" : null
      }  glassmorphism`}
    >
      <Link
        href={"/"}
        className="text-lg font-extrabold text-white font-caption select-none flex gap-x-2 items-center"
      >
        <div className="flex gap-x-2 items-center">
          <div className="relative w-10 h-10 rounded-full flex items-center justify-center pt-2 bg-white overflow-hidden">
            <Image
              ...
              priority
              alt="profil"
              className="object-contain scale-150 w-full h-full"
            />
          </div>
          <span className="text-sm"> CodesofC </span>
        </div>
      ...
      <div
        className={`w-[90%] md:w-auto ${
          isOpen
            ? "translate-y-[13%] md:translate-y-0"
            : "-translate-y-[300%] md:translate-y-0"
        } z-20 shadow-xl md:shadow-none transition-all absolute left-1/2 -translate-x-1/2 md:translate-x-0 -bottom-[330%] md:relative md:left-0`}
      >
